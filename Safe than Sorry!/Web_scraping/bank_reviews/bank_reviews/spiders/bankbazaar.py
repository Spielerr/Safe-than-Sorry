...
class Bank(scrapy.Spider):
	name = "bank"
	page = 0
	start_urls = ["https://www.bankbazaar.com/reviews/syndicate-bank/all-products.html"]
	def parse(self, response):
		...
		with open('data5.csv', mode='a+') as file:
			writer = csv.writer(file, delimiter='|', quotechar='"', quoting=csv.QUOTE_MINIMAL)
			for i in range(len(reviews)):
				writer.writerow([acc[i], title[i], reviews[i], username[i], place[i], date[i]])
		...
		while (Bank.page <= (last_page-1)):
			Bank.page += 1
			next_page = "https://www.bankbazaar.com/reviews/syndicate-bank/all-products.html" + "?reviewPageNumber=" + str(Bank.page)
